...
Provide common helpers:
  ...
  $ get_ssid_enable() { R "ba-cli -j -l WiFi.SSID.?0 | jsonfilter -e @[0]'[@.Alias != \"ep2g0\" && @.Alias != \"ep5g0\" && @.Alias != \"ep6g0\"].Enable'" | LC_ALL=C sort;}
...
Set 5GHz channel to a non DFS one to make 5GHz instantly available and not wait for the CAC delay:
...
Check that wireless is disabled. the Agent is supposed to change this field in the datamodel:
...
If Enable flag is 0 but Status is not Down, there may be a problem with pwhm:
