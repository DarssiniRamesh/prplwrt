...
Create ReqOption with even length and hexBinary value:
  ...
  $ R /usr/bin/ba-cli 'Device.DHCPv4.Client.1.ReqOption.+ {Enable=0, Alias=TestReq1, Value=12ab}' | grep -Ev '^>|^$'
  Device.DHCPv4.Client.1.ReqOption.12.
  Device.DHCPv4.Client.1.ReqOption.12.Alias="TestReq1"

Try to set Value parameter
  $ R /usr/bin/ba-cli 'Device.DHCPv4.Client.1.ReqOption.12.Value=12ba' | grep -Ev '^>|^$'
...
Create ReqOption with odd length and hexBinary Value:
  $ R /usr/bin/ba-cli 'Device.DHCPv4.Client.1.ReqOption.+ {Enable=0, Alias=TestReq2, Value=12abc}' | grep -Ev '^>|^$'
...
Create ReqOption with even length and non-hexBinary Value:
  $ R /usr/bin/ba-cli 'Device.DHCPv4.Client.1.ReqOption.+ {Enable=0, Alias=TestReq2, Value=12ah}' | grep -Ev '^>|^$'
  ERROR: add Device.DHCPv4.Client.1.ReqOption. failed (10 - invalid value)

Remove test ReqOption:
  $ R /usr/bin/ba-cli 'Device.DHCPv4.Client.1.ReqOption.12.-' | grep -Ev '^>|^$'
  Device.DHCPv4.Client.1.ReqOption.12.
