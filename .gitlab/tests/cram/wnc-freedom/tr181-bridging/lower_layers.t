...
Check lan bridge ports in HLAPI:
  $ R /usr/bin/ba-cli 'Device.Bridging.Bridge.lan.Port.?' | grep 'LowerLayers'
...
Replace a port with eth1:
  $ R /usr/bin/ba-cli 'Device.Bridging.Bridge.lan.Port.5.LowerLayers=Device.Ethernet.Interface.1' >/dev/null
  $ R /usr/bin/ba-cli 'Device.Bridging.Bridge.lan.Port.5.Name?' | grep -v '^>'
...
Restore:
  $ R /usr/bin/ba-cli 'Device.Bridging.Bridge.lan.Port.5.LowerLayers=Device.Ethernet.Interface.5' | grep -v '^>'
