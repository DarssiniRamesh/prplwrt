...
Unmount key partitions:
  ...
  $ R "for p in \$(mount |grep /dev/sda |cut -d\" \" -f3); do umount -f \$p; done"

Create and format the partitions:

  $ R "/tmp/storageservice.sh create_partition"

Force tr140-storageservice restart to detect newly created partitions:

  $ R "/tmp/storageservice.sh restart_service"

Check filesystem are correctly mounted:

  $ R "/tmp/storageservice.sh check_partitions"
...
# Configure a share for ext3 partition:
  ...
  $ R "/tmp/storageservice.sh create_share"
...
Verify the new password:
  ...
  $ smbclient \\\\${TARGET_LAN_IP}\\ext3 -U 'prpluser%newprplpassword' --command=ls 2> /dev/null | grep ext3 > /dev/null 2> /dev/null
