...
Check PeriodicFileTransfer error code reporting for various failure scenarios:
  ...
  $ R 'ba-cli "Device.PeriodicFileTransfer.Transfer.cram-Transfer-1.Enable=0"' > /dev/null
  $ R 'ba-cli "Device.PeriodicFileTransfer.Transfer.cram-Transfer-1.UploadInterval=86400"' | grep -v '>'; sleep 1
  Device.PeriodicFileTransfer.Transfer.(.+). (re)
  Device.PeriodicFileTransfer.Transfer.(.+).UploadInterval=86400 (re)
  ...
  [
      {
          data = "Transfer ended with error code (9013)"
      }
  ]
