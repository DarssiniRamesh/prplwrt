...
Check that USP controller is configured after option-125 has changed:
  ...
  $ R "obuspa -f /etc/obuspa.db -c get Device.LocalAgent.Controller.[Alias==usp_discovery_cdrouterTestController]." | awk -F'Device.LocalAgent.Controller.[0-9]+.' '{print $2}'
  ...
  MTP.1.WebSocket.CurrentRetryCount => 0
...
Check that second USP controller is configured after option-125 updated with new EndpointID:
  ...
  MTP.1.UDS.UnixDomainSocketRef => 
  MTP.1.UDS.USPServiceRef => 
  $ R "obuspa -f /etc/obuspa.db -c get Device.LocalAgent.Controller.[Alias==usp_discovery_cdrouterTestControllerNEW]." | awk -F'Device.LocalAgent.Controller.[0-9]+.' '{print $2}'
...
Check that USP controller is configured with 'UseSingleControllerInstance' enabled:
  ...
  $ R "sleep ${USP_DISCOVERY_CONTROLLER_CONFIGURE_TIMEOUT}"
  $ R "obuspa -f /etc/obuspa.db -c get Device.LocalAgent.Controller.[Alias==usp_discovery]." | awk -F'Device.LocalAgent.Controller.[0-9]+.' '{print $2}'
  Alias => usp_discovery
  Enable => true
  EndpointID => cdrouterTestController
  InheritedRole => 
